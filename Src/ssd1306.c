...
uint8_t SSD1306_Init(void) {
	...
	*/
	...
	SSD1306_WRITECOMMAND(0xD5); //--set display clock divide ratio/oscillator frequency
	SSD1306_WRITECOMMAND(0x80); //display off
	SSD1306_WRITECOMMAND(0xA8); //Set Memory Addressing Mode   
	SSD1306_WRITECOMMAND(0x3F); //00,Horizontal Addressing Mode;01,Vertical Addressing Mode;10,Page Addressing Mode (RESET);11,Invalid
	SSD1306_WRITECOMMAND(0xD3); //Set Page Start Address for Page Addressing Mode,0-7
	SSD1306_WRITECOMMAND(0x00); //Set COM Output Scan Direction
	SSD1306_WRITECOMMAND(0x40); //---set low column address
	SSD1306_WRITECOMMAND(0x8D); //---set high column address
	SSD1306_WRITECOMMAND(0x14); //--set start line address
	SSD1306_WRITECOMMAND(0x20); //--set contrast control register
	SSD1306_WRITECOMMAND(0x00);//
	SSD1306_WRITECOMMAND(0xA1); //--set segment re-map 0 to 127
	SSD1306_WRITECOMMAND(0xC8); //--set normal display
	SSD1306_WRITECOMMAND(0xDA); //--set multiplex ratio(1 to 64)
	SSD1306_WRITECOMMAND(0x12);
	SSD1306_WRITECOMMAND(0xCF); //0xa4,Output follows RAM content;0xa5,Output ignores RAM content
	SSD1306_WRITECOMMAND(0xD9); //-set display offset
	SSD1306_WRITECOMMAND(0xF1); //-not offset
	SSD1306_WRITECOMMAND(0xD8); //--set divide ratio
	SSD1306_WRITECOMMAND(0x40); //--set pre-charge period
